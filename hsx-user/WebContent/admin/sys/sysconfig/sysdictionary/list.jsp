		...
		<script type="text/javascript">
			jQuery(function($) {
				...
				var jqGridJSON = {
								...
								url:urls.selectSysDictionaryList.url,
								datatype: "json",
								mtype:"POST",
								height: 350,
								colNames:['ID','数据字典名称','数据字典描述', '数据字典URL','数据字典类型','创建时间','修改时间','operation'],
								colModel:[
									{name:'id',index:'id', width:100, sorttype:"int", editable: true},
									{name:'dicName',index:'dicName',width:105, editable:true, sortable:false},
									{name:'dicDsc',index:'dicDsc', width:105,editable: true, sortable:false},
									{name:'dicUrl',index:'dicUrl', width:105, editable: false, sortable:false},
									{name:'dicType',index:'dicType', width:105, editable: false, sortable:false},
				...
				jQuery(grid_selector).jqGrid('navGrid',pager_selector,
					{ 	//navbar options
						...
						addfunc : addSysDictionaryModal, 
						delfunc : deleteSysDictionary,
						editfunc : editSysDictionaryModal, 
					},
					{
						//edit record form
						width: 1120,
...
/* 					buttons = form.next().find('.navButton a');
		...
		<script type="text/javascript">
			var roleId = "";
			<%-- 添加start --%>	
			var addSysDictionaryModal = function(){
			  	$("#myModal").modal({
			  	  	remote:ctx+"/admin/sys/sysconfig/sysdictionary/add.jsp",
			  	})
			}
			function saveSysDictionary(){
				var datas = urls.saveSysDictionary;
				datas.type = "POST";
			...
			<%-- 修改end --%>
			<%-- 修改start --%>
			var editSysDictionaryModal = function(rowId){
/* 			  	var form = $("form.updateForm");
			...
			function updateSysDictionary(){
			  	var datas = urls.updateSysDictionary;
				datas.type = "POST";
				datas.data = $("form.updateForm").serializeArray();
				datas.success = function(res){
				  	if(res.code == 1){
			...
			<%-- 删除start --%>
			var deleteSysDictionary = function(rowId){
 				var datas = urls.deleteSysDictionary;
...
/* 		  		var flag = false;
			...
			$('#myModal').on('loaded.bs.modal', function (e) {
				...
				if($.trim(roleId) != "" && !isNaN(roleId)){
				  	var modal = $(this);
					var datas = urls.selectSysDictionaryAndSetData;
					datas.url = (datas.url.indexOf("?")>0?datas.url.substring(0,datas.url.indexOf("?")):datas.url) + "?id="+roleId;
					datas.type="POST";
					datas.success = function(res){
						if(res.code == 1){
						  	var dictionary = res.obj;
						  	modal.find("input[name=id]").val(""+dictionary.id);
						  	modal.find("input[name=dicName]").val(dictionary.dicName);
						  	modal.find("input[name=dicDsc]").val(dictionary.dicDsc);
						  	modal.find("input[name=dicUrl]").val(dictionary.dicUrl);
						  	//modal.find("input[name=dicParentid]").val(dictionary.dicParentid);
						  	modal.find("select[name=dicType]").val(dictionary.dicType);
						  	modal.find("button#submitButton").attr("onClick","updateSysDictionary()");
						}else{
						  	alert(res.desc);
						}
					}
